...
import { CHENNAI_LOCALITIES } from '@/lib/data'

const citySlug = (city) => city.toLowerCase().replace(/\s+/g, '-')

/**
 * "Serving All Areas in Chennai" — the locality mesh for a service.
 *
 ...
 * crawlers.
 *
 * @param {string}  serviceSlug  e.g. '/chennai/steel-gates'
 * @param {string}  serviceName  e.g. 'Steel Gates'
 * @param {string} [cityName]    current locality, undefined on the pillar page
 */
export default function ServiceAreas({ serviceSlug, serviceName, cityName }) {
            ...
            <div className="max-w-6xl mx-auto px-4">
                <div className="text-center mb-12">
                    ...
                    <h2 className="text-3xl md:text-4xl font-bold text-fg font-heading">
                        Serving All Areas in Chennai
                    </h2>
                    <p className="text-[15px] text-fg-muted mt-4 max-w-2xl mx-auto leading-relaxed">
                        {serviceName} delivered across Chennai and the surrounding industrial belt.
                ...
                <ul className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                    {CHENNAI_LOCALITIES.map((city) => {
                        const isCurrent = current === city.toLowerCase()
                        ...
                        return (
                            <li key={city}>
                                <Link
                                    href={`${serviceSlug}-in-${citySlug(city)}`}
                ...
                {cityName && (
                    <div className="text-center mt-10">
                        ...
                        >
                            ← Back to {serviceName} in Chennai
                        </Link>
                    </div>
                )}
            </div>
