...
function cityDeityRedirects() {
    ...
    /*
     * Matched against the source text, so it is coupled to how lib/gods.js is
     * written. It already broke once: the array was renamed to RAW_GODS when
     * `faith` was added, and a config that greps a file for a variable name
     * fails at build with nothing pointing at the rename. Hence the explicit
     * error below rather than a null dereference, and hence matching either
     * name.
     */
    const godSrc = readFileSync('./lib/gods.js', 'utf8')
    const match = godSrc.match(/const (?:RAW_)?GODS = \[([\s\S]*?)\n\]/)
    if (!match) {
        throw new Error(
            'next.config.js: could not find the GODS array in lib/gods.js. ' +
                'If it was renamed, update cityDeityRedirects() to match — the ' +
                '200 city redirects are generated from it.'
        )
    }
    /*
     * Hindu keys only, and the omission is deliberate.
     *
     * These rules exist to catch ~200 city deity URLs that were generated by
     * mistake and got indexed before the mistake was found. Nothing in the city
     * routes generates them any more, so the set is closed: the Christian and
     * Islamic subjects added later never had a city page, were never crawled,
     * and a redirect for a URL that has never existed is dead config.
     *
     * A row carries `faith:` only when it is not Hindu — lib/gods.js defaults
     * it — so the absence of that word is the test.
     */
    const keys = match[1]
        .split('\n')
        .filter((line) => line.includes("key:") && !line.includes('faith:'))
        .map((line) => line.match(/key:\s*'([^']+)'/))
        .filter(Boolean)
        .map((m) => m[1])
