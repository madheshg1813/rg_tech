...
/**
 * Persist a website lead.
 *
 * Server-only. Sanity is the system of record for enquiries so a lead is never
 * lost because an external script is misconfigured — forwarding to Google Sheets
 * is treated as a best-effort extra, not the source of truth.
 */
export async function createEnquiry(doc) {
    if (!sanityClient || !READ_TOKEN) {
        throw new Error('Sanity is not configured for writes')
    }
    return sanityClient.create({ _type: 'enquiry', ...doc })
}
