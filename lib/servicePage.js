...
import { getCity, serviceUrl, serviceKeyOf, localitySlug } from '@/lib/cities'
...
export function resolveService(citySlug, slugArray) {
    ...
    let cityName = null
    if (localityPart) {
        // Only accept localities this city actually serves, so a stray slug
        // 404s instead of generating an orphan page for a place we do not cover.
        const match = city.localities.find(
