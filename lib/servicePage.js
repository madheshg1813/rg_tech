...
import { getGod, GOD_SUFFIX } from '@/lib/gods'
 ...
 */
/**
 * Deity pages share the city catch-all: /{city}/{god}-laser-cutting-services.
 * Checked before services so a god key never collides with a service key.
 */
export function resolveGod(citySlug, slugArray) {
    const city = getCity(citySlug)
    if (!city || slugArray.length !== 1) return { city: null, god: null }
    const part = slugArray[0]
    if (!part.endsWith(`-${GOD_SUFFIX}`)) return { city, god: null }
    const god = getGod(part.slice(0, -(GOD_SUFFIX.length + 1)))
    return { city, god }
}
...
export async function buildServicePage(citySlug, params) {
    ...
    const serviceSchema = {
        ...
        "url": pageUrl,
        /*
         * The sub-services catalogue, on the pillar only — markup has to
         * describe what is actually on the page, and ServiceClient renders that
         * section for pillars alone. Emitting it on 822 locality pages would
         * also be 822 identical catalogues.
         */
        ...(!cityName && content.subServices?.length
            ? {
                "hasOfferCatalog": {
                    "@type": "OfferCatalog",
                    "name": `${content.name} in ${city.name}`,
                    "itemListElement": content.subServices.map((sub) => ({
                        "@type": "Offer",
                        "itemOffered": {
                            "@type": "Service",
                            "name": sub.name,
    ...
    /*
     * subServices render on the pillar only, so drop them from the locality
     * payload. ServiceClient is a client component, which means everything on
     * `content` is serialised into the RSC flight data whether it is rendered
     * or not — leaving them in shipped 39 unused entries, and every one of
     * those keyword strings, into the HTML of all 822 locality pages.
     */
    const { subServices, ...contentWithoutSubServices } = content

    return {
        city,
        content: cityName ? contentWithoutSubServices : content,
        cityName,
        cityIndex,
        pathName,
        graph,
